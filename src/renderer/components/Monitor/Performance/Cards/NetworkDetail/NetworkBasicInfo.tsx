...
export const NetworkBasicInfo: React.FC<NetworkBasicInfoProps> = React.memo(({ networkInfo }) => {
  // 获取迷你图表配置
  const getMiniChartOption = useMemo(() => {
    return (data: { timestamp: number; rxSpeed: number; txSpeed: number }[], ifaceName: string) => ({
      grid: {
        top: 5,
        right: 5,
        bottom: 5,
  ...
  // 获取带宽趋势图配置
  const bandwidthTrendOption = useMemo(() => {
    const data = networkInfo.history;
    return {
      grid: {
        top: 60,
        right: 20,
  ...
  const renderInterfaces = useMemo(() => {
    return networkInfo.interfaces.map((iface) => (
  ...
  }, [networkInfo.interfaces, networkInfo.history, getMiniChartOption]);
