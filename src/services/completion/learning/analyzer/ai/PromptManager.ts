...
【输出规则】
■ 保持原有JSON结构,不要修改,不要任何解释，不要任何备注，不要带markdown格式
■ command字段必须为纯命令文本
■ 通过以下字段传递信息：
   - context：注明"预测依据"
...
请：
1. 合并相似命令（保留最高频的），低频命令不返回,无法识别的命令不返回
2. 修正明显错误命令
...
要求：
- 预测命令作为独立pattern项，但不要猜测假目录或者假文件，除非有历史命令为证
- 错误修正需保留原命令frequency
- 不要任何解释，不要任何备注，前面不要带markdown格式（比如~~~json）,专注json文本格式
- 所有command字段保持纯净`;
